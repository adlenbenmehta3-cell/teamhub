...
export function ReportsModule({ user }: Props) {
 ...
 ? formatArabicDate(new Date()): formatEnglishDate(new Date());
 ...
 className={
 todayData?.myReport
 ? "border-border": "border-border bg-muted/50"
 }
 >
 ...
 <Folder className="w-4 h-4 text-primary mt-0.5" />
 ...
 {member.user.name.split(" ").slice(0, 2).map((s: string) => s[0]).join("")}
 </AvatarFallback>
 </Avatar>
 <div>
 ...
 {lang === "ar"
 ? formatArabicDate(r.date): formatEnglishDate(r.date)}
 </p>
 {r.driveLink && (
 <a
 href={r.driveLink}
...
function ReportField({
 ...
 <div
 className={`mt-0.5 ${
 warning ? "text-primary": "text-primary"
 }`}
 >
 {icon}
 </div>
