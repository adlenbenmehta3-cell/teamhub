...
export function TasksModule({ user }: Props) {
 ...
 toast.success(
 data.early
 ? t("tasks.completedEarly", { points: data.points }): t("tasks.completedMsg", { points: data.points })
 );
 loadTasks();
 } catch {
 ...
 {filter === "open"
 ? t("tasks.noOpenTasks"): filter === "completed"
 ? t("tasks.noCompletedTasks"): t("tasks.noTasks")}
 </p>
 </CardContent>
 </Card>
 ): (
 ...
 className={
 isOverdue
 ? "border-destructive/40": task.status === "COMPLETED"
 ? "border-border opacity-75": "border-border"
 }
 >
