...
export function KnowledgeBaseModule({ user }: Props) {
 ...
 className={
 filter === "all"
 ? "bg-primary text-primary-foreground hover:bg-primary/90 hover:bg-accent": "border-border text-primary"
 }
 >
 ...
 {filter === "all"
 ? t("kb.noEntries"): t("kb.noEntriesFiltered", {
 ...
 {entries.filter((e) => filter === "all" || e.category === filter).map((entry) => {
 ...
 variant="ghost"
 size="sm"
 className="mt-2 text-destructive hover:bg-destructive/5"
